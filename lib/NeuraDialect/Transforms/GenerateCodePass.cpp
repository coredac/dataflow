  ...
  void runOnOperation() override {
        ...
        if (auto const_op = mlir::dyn_cast<neura::ConstantOp>(op)) {
          ...
          if (val_attr) {
            if (auto int_attr = mlir::dyn_cast<IntegerAttr>(val_attr)) {
              ...
              op_obj["constant_value"] =
                  std::to_string(float_attr.getValueAsDouble());
            }
          }
        }
          ...
          for (Attribute attr : attr_array) {
            if (auto loc = mlir::dyn_cast<DictionaryAttr>(attr)) {
              ...
              if (auto resource_attr =
                      mlir::dyn_cast<StringAttr>(loc.get("resource"))) {
                loc_obj["resource"] = resource_attr.getValue().str();
              }
              if (auto timestep_attr =
                      mlir::dyn_cast<IntegerAttr>(loc.get("time_step"))) {
    ...
    if (ec) {
      getOperation()->emitError(
          "Failed to open 'generated-instructions.json' for writing: " +
          ec.message());
