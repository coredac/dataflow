...
SmallVector<CgraShape> mlir::taskflow::getAllPlacementShapes(int cgra_count) {
  ...
  for (const auto &base : base_non_rect) {
    ...
    // give low collision rates for small integer coordinate sets (CGRA counts
    // up to 4, grid up to 4×4).  The combination 131*prev + col*17 + row
    // distributes col and row contributions into distinct bit ranges, making
    // accidental hash collisions between different shapes very unlikely.
