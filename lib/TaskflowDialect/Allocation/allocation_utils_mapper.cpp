...
//
// Implements the internal TaskMapper and runAllocateCgraToTask used by
// AllocateCgraToTaskPass to place Taskflow tasks onto the 2D multi-CGRA grid.
//
//===----------------------------------------------------------------------===//
...
//===----------------------------------------------------------------------===//
/// Maps a task-memory graph onto a 2D multi-CGRA grid.
///
...
/// Iterates until SRAM assignments converge (routing-critical-path-first
/// ordering).
class TaskMapper {
public:
...
private:
  ...
  //
  // The `tile_shape` attribute set by the upstream
  ...
  //
  // If no `tile_shape` attribute is present (e.g. cgra_count == 1 or the task
  ...
  TaskPlacement findBestPlacement(TaskNode *task_node, int cgra_count,
    ...
    if (auto attr = task_node->op->getAttrOfType<StringAttr>("tile_shape")) {
      StringRef tile_shape_str = attr.getValue();
      if (!tile_shape_str.empty()) {
        CgraShape base = parseTileShapeToBase(tile_shape_str, cgra_count);
        shapes_to_try = rotationsOf(base);
      }
    }

    // Fallback: no tile_shape attribute, enumerates all valid shapes.
  ...
  // Parses a `tile_shape` IR attribute string into a base CgraShape.
  // Accepts "NxM" (rectangular) and "NxM[(c0,r0)(c1,r1)...]" (non-rectangular).
  CgraShape parseTileShapeToBase(StringRef tile_shape, int cgra_count) {
    size_t bracket_pos = tile_shape.find('[');
    auto [rows_str, rest] = tile_shape.split('x');
    int rows = 1, cols = 1;
  ...
  /// downstream.  Placing them first (routing-critical-path-first) ensures
  /// that:
  ...
  int calculateDepth(TaskNode *node, DenseMap<TaskNode *, int> &depth_cache) {
    ...
    // Memory dependencies: Producer -> Mem -> Consumer write-after-read (WAR)
    // chains.  Currently only WAR edges are traversed because streaming-fusion
    // has already eliminated true RAW dependencies (they become SSA edges), and
    // WAW dependencies between independent tasks do not create ordering
    // constraints for placement purposes.  Extending to full WAW/RAW coverage
    // is left as future work once more precise alias analysis is available.
