...
public:
  ...
  void place(func::FuncOp func) {
    ...
    llvm::outs() << "=== CGRA Placement ===\n";
    llvm::outs() << "Grid size: " << grid_rows_ << " x " << grid_cols_ << "\n";
    llvm::outs() << "Tasks to place: " << tasks.size() << "\n\n";
    ...
    for (size_t idx : placement_order) {
      ...
      llvm::outs() << "Placed " << tasks[idx].getTaskName() << " at ";
      for (size_t i = 0; i < placement.cgraCount(); ++i) {
        auto pos = placement.cgra_positions[i];
        llvm::outs() << "(" << pos.row << "," << pos.col << ")";
        if (i < placement.cgraCount() - 1) llvm::outs() << "+";
      }
