...
test_path = os.path.join(PROJECT_ROOT, "test/e2e/fir/fir_kernel_vec.mlir")
...
for j in range(s + 1, len(lines)):
    if not (lines[j].strip().startswith("// MAPPING") or lines[j].strip().startswith("//MAPPING") or lines[j].strip() == "//"):
        # Double check if it's really the end (e.g. skip single // line)
        if lines[j].strip() == "//" and j+1 < len(lines) and lines[j+1].strip().startswith("// MAPPING"):
            continue
        e = j
        break
