...
def update_file(test_file):
    ...
    is_e2e = "e2e" in test_file or "honor_arch" in test_file or "c2llvm2mlir" in test_file
    
    if is_e2e:
        # Check if it wants vectorization
        vec_flag = "-fno-vectorize"
        if "vec" in test_file:
            vec_flag = "" # Allow vectorization
            
        if "fir" in test_file:
            cpp_src = "/home/x/shiran/Project/dataflow/test/benchmark/CGRA-Bench/kernels/fir/fir_int.cpp"
            cmd = [clang_plus, "-S", "-emit-llvm", "-O3", "-fno-unroll-loops", "-o", tmp_ll_full, cpp_src]
            if vec_flag: cmd.insert(5, vec_flag)
            subprocess.run(cmd)
        elif "relu" in test_file:
            c_src = "/home/x/shiran/Project/dataflow/test/benchmark/CGRA-Bench/kernels/relu/relu.c"
            subprocess.run([clang, "-S", "-emit-llvm", "-O3", vec_flag, "-fno-unroll-loops", "-std=c11", "-I", os.path.dirname(c_src), "-DSMALL_DATASET", "-o", tmp_ll_full, c_src])
            ...
            subprocess.run([clang_plus, "-S", "-emit-llvm", "-O3", vec_flag, "-fno-unroll-loops", "-o", tmp_ll_full, cpp_src])
        elif "c2llvm2mlir/nested_loop" in test_file:
             base = os.path.dirname(test_file)
             subprocess.run([clang_plus, "-S", "-emit-llvm", "-o", tmp_ll_full, f"{base}/kernel.cpp"])
        else:
             print(f"Unsupported e2e test: {test_file}")
             return

        # Fix LLVM 20 compatibility
    ...
    def get_lines(filename, prefix_base, limit=60):
        ...
        for line in content_lines:
            l_strip = line.strip()
            if not l_strip: continue # Skip completely empty lines to avoid FileCheck errors
            is_first = (len(res) == 0)
            p = f"// {prefix_base}:" if is_first else f"// {prefix_base}-NEXT:"
            if prefix_base == "ASM" and "PE(" in line: p = "// ASM:"
            res.append(f"{p} {l_strip}")
        return res

    mapping_lines = []
    with open(tmp_mapping, 'r') as f:
        mapping_content = f.read()
    # Use re.DOTALL to match across lines and .*? for non-greedy match
    match = re.search(r'func.func @(kernel|loop_test).*?attributes \{.*?mapping_info = \{.*?\}.*?\} \{', mapping_content, re.DOTALL)
    if match:
        start = match.start()
        body = mapping_content[start:].splitlines()[:150]
        ...
        print(f"Warning: Could not find mapping_info in {tmp_mapping}")
    
    yaml_lines = get_lines(tmp_yaml, "YAML", limit=500)
    asm_lines = get_lines(tmp_asm, "ASM", limit=1200)
    
    if not yaml_lines: print(f"Warning: No YAML lines extracted from {tmp_yaml}")
    if not asm_lines: print(f"Warning: No ASM lines extracted from {tmp_asm}")
    ...
    for line in lines:
        ...
        for key in ["MAPPING", "YAML", "ASM"]:
            if line.strip().startswith(f"// {key}:") or line.strip().startswith(f"// {key}-NEXT:"):
                if not added[key]:
