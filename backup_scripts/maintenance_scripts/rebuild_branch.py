...
# Step 2: Define static parts based on Step 3168
base_runs = [
...
with open(test_path, 'w') as f:
    f.writelines(base_runs)
    f.writelines(new_runs)
    f.write(func_body)
    f.write(checks)
    f.write("\n")
    f.write(f"// MAPPING:        {header}\n")
    f.write("\n")
    f.write("// YAML:      array_config:\n")
    for i, l in enumerate(yaml[1:]):
        f.write(f"// YAML-NEXT: {l.strip()}\n")
    f.write("\n")
    f.write(f"// ASM:      {asm[0].strip()}\n")
    for i, l in enumerate(asm[1:]):
        f.write(f"// ASM-NEXT: {l.strip()}\n")
